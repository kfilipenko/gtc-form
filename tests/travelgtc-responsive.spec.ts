...
test.describe('TravelGTC responsive public site', () => {
  ...
  test('CRM customer card presents registration data and linked application history to the team', async ({ page }) => {
    ...
    await page.route('**/api/travelgtc/v1/crm/customers**', async (route) => {
      const requestUrl = new URL(route.request().url());
      if (requestUrl.pathname.endsWith(`/${contactId}`)) {
        await route.fulfill({
          contentType: 'application/json',
          body: JSON.stringify({
            ok: true,
            customer: {
              ...
              display_name: 'Дмитрий Викторович',
              registration_email: 'dmitry@example.com',
              registration_phone: '+79990000000',
              registration_primary_channel: 'email',
              account_status: 'active',
              registered_at: '2026-08-05T10:00:00.000Z',
              relationship_status: 'active',
              assigned_to: 'Константин',
            },
            roles: [{ role_code: 'traveler' }],
            leads: [{ lead_id: '22222222-2222-4222-8222-222222222222', primary_interest: 'become_travel_advantage_member', stage: 'membership_interest', created_at: '2026-08-05T10:01:00.000Z', summary: 'Хочу узнать о Membership.' }],
            tasks: [{ title: 'Подготовить консультацию', priority: 'high', created_at: '2026-08-05T10:02:00.000Z' }],
            notes: [{ created_at: '2026-08-05T10:03:00.000Z', body: 'Перезвонить после изучения условий.' }],
            interactions: [{ created_at: '2026-08-05T10:04:00.000Z', interaction_type: 'ai_chat', body: 'Хочу понять следующий шаг.' }],
            audit: [],
          }),
        });
        return;
    ...
    await page.goto(`/crm/customers/?id=${contactId}`, { waitUntil: 'domcontentloaded' });
    const customerCard = page.getByLabel('Карточка клиента');
    await expect(customerCard.getByRole('heading', { name: 'Дмитрий Викторович' })).toBeVisible();
    await expect(customerCard.getByText('dmitry@example.com')).toBeVisible();
    await expect(customerCard.getByText('+79990000000')).toBeVisible();
    await expect(customerCard.getByRole('heading', { name: 'Заявки клиента' })).toBeVisible();
    await expect(customerCard.getByText('Хочу понять следующий шаг.')).toBeVisible();

    await page.setViewportSize({ width: 390, height: 844 });
    const mobileDimensions = await measureHorizontalOverflow(page);
    await assertNoHorizontalOverflow(mobileDimensions.viewportWidth, Math.max(mobileDimensions.documentScrollWidth, mobileDimensions.bodyScrollWidth));
  });
});
