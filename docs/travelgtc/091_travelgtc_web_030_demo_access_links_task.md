# TRAVELGTC-WEB-030 - Travel Advantage Access Links Task
...
Add official Travel Advantage access links to the TravelGTC site and to Mira's knowledge/context so users can explore the product before paid registration.
...
Free Guest Pass should become a softer funnel step before purchase.

Instead of pushing only the registration link, Mira and the site can offer:

1. see the platform first;
2. compare what Free Guest Pass and VIP Membership show;
...
```text
Interest -> Free Guest Pass -> Questions -> Membership fit -> Purchase intent -> Referral registration
```
...
```text
Free Guest Pass помогает посмотреть Travel Advantage до регистрации и оплаты. VIP Membership ведёт к платному VIP-членству. Доступ, условия, состав предложений и доступность сервисов необходимо проверять на официальных страницах Travel Advantage.
```

## 4. Mira Agent Requirements

Mira may provide access links when the user:

1. asks what Travel Advantage looks like;
2. wants to see the platform before registration;
3. hesitates before paid Membership;
4. asks for Free Guest Pass, free access, trial or examples;
5. compares Membership levels.

Mira must clearly distinguish:

1. Free Guest Pass access;
2. official paid registration through `https://www.mwrlife.com/KFilip909`;
3. consultation / CRM follow-up through TravelGTC.

## 5. CRM Requirements

When Mira gives an access link, the interaction should remain in CRM history.

Future optional enhancement:

1. add metadata flag `travel_advantage_access_link_shared`;
2. raise lead stage to `first_look_shared` only if we decide to extend CRM stages.
...
1. these links are provided by the Project Owner as active Travel Advantage access links;
2. TravelGTC will not represent Free Guest Pass or VIP Membership as guaranteed availability, price or final Membership condition;
3. Mira must not call VIP Membership a trial page and must not promise that Free Guest Pass equals paid account inventory or final booking terms;
...
1. access links are visible on approved public page locations;
2. links open in a new tab with `rel="noopener"`;
3. Mira can mention both access links in relevant answers;
...
Approve publishing both access links publicly:
