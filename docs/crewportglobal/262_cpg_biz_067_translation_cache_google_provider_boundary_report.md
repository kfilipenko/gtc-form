...
Он добавил protected credential source decision и validator для `GOOGLE_APPLICATION_CREDENTIALS` / `GOOGLE_CLOUD_PROJECT`.

Следующий рекомендуемый этап после CPG-BIZ-068:

```text
CPG-BIZ-069 - Translation cache Google client implementation behind protected adapter
```

Цель: подключить реальный Google client только после настройки protected credentials в deployment/server environment.
