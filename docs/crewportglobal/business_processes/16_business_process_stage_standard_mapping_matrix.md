...
- Version: 3.10
- Date: 2026-06-10
- Status: Synchronized after CPG-BIZ-123 shipowner-agent appointment package update
...
| CPG-BIZ-123 | Shipowner-agent agreement checkbox/radio template | Updated full RU template for future portal generation and agent appointment activation, including agreement/POA/appendix gate, no-fee/SEA coordination boundaries, exclusive delegated management and notification ledger clauses |
...
| P1 | Participant governance notification ledger API/UI implementation | Required after CPG-BIZ-126 and the CPG-BIZ-123 shipowner-agent appointment package so seafarers and shipowners can claim agent-created records, appoint/reject/replace representatives and receive durable audit-visible notifications for authority, representative agreement/POA, delegation, document-stage and obligation-stage milestones. |
...
```text
CPG-BIZ-123 shipowner-agent agreement template is updated as a future portal appointment package, and CPG-BIZ-126 remains the governing standard for participant representative appointment and notification.
```
...
Goal: implement the first runtime slice of the CPG-BIZ-126 model, using the CPG-BIZ-123 shipowner-agent appointment package as the first activation scenario. When a shipowner appoints an agent from a signed agreement, POA/authority document and mandatory appendices, or when any agent creates, claims, receives, loses or replaces management of a represented participant object, confirms authority, prepares/generates/signs/rejects a document or completes/blocks an obligation milestone, the represented participant, new agent, previous agent and Platform Administration / Control must receive safe computed notifications/tasks. The implementation must persist notification ledger records, must not leak unrelated object data, must preserve the one-active-manager rule, must lock participant operational editing while delegation is active and must keep contract-critical work blocked until party activation or enhanced authority is verified.
