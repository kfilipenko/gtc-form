...
- Version: 3.17
- Date: 2026-06-19
- Status: Synchronized after CPG-BIZ-137 agent-assisted direct contract drafting task
...
| CPG-BIZ-137 | Agent-assisted direct contract drafting | Defines the approved main working mode where an authorized agent prepares permitted direct SEA draft variables inside the existing Contract Agreement Workspace, while verified source facts remain database-filled and final party review/signature remains required. |
...
| Contract Agreement Workspace | CC-14 / CF-14 | Populated agreement with embedded condition fields, contract kind, source adapters, agent-assisted draft versions, required appendices and party approvals | Group 1, Group 4, Group 5, represented agent where scoped | CPG-BIZ-091/092/093/094/095/096/097/098A/098B/098C/098D/099/100/102, CPG-BIZ-125, CPG-BIZ-132, CPG-BIZ-134, CPG-BIZ-135, CPG-BIZ-136, CPG-BIZ-137, BP-014 | Implemented for direct SEA foundation: workspace model, clause library, catalogs, source-first prefill rule, exact shortlist candidate link, runtime schema migration 018, employer `propose_contract` API/UI creation/reuse, detail view and verified source-prefill page `/contracts/workspace/`. CPG-BIZ-135 identifies that the current code is still SEA-specialized and must be generalized by `contract_kind` before agent agreements are generated through the same script. CPG-BIZ-137 defines the next practical direct-SEA slice: an authorized agent may prepare permitted draft variables and submit a versioned populated draft to the real parties for approval. | Implement agent-scoped access to eligible direct SEA workspaces, embedded field editing, draft versioning/preview hash, party review/correction requests, multi-template registry/API/UI, appendix/evidence checklist, audit evidence, party-review readiness guard and generated instance script. |
...
| P1 | Agent-assisted direct contract drafting workflow | Required for the accepted agent working mode: agents must be able to open eligible direct SEA workspaces, prepare permitted draft variables and send versioned drafts to the parties without replacing source-first data or party approvals. |
