...
| CPG-BIZ-115 | Agent API authority and management scope plan | Defines that an agent may create any supported object request, but may manage the created/linked object only after verified authority evidence and active object assignment; authority must be visible in agent lists and participant cards |
...
| Agent onboarding, object creation and scope | CF-00A / CF-00B / CF-00C | Agent organization, agent-created object request, assignment, account/object claim | Platform Administration / Control, assigned agent organization | BP-012, BP-013, CPG-BIZ-111, CPG-BIZ-112, CPG-BIZ-113, CPG-BIZ-114, CPG-BIZ-115 | Runtime migration 020 implemented and verified; `/agents/` page and `Agents` navigation shell implemented; process documents define same-rule GTC/external agent model, duplicate/claim boundary and the CPG-BIZ-115 rule that creation requests do not grant management access until verified authority and active assignment exist | API skeleton, verified-authority guard, task-computation scope update, duplicate/claim notification workflow and reassignment workflow. |
...
| P1 | Agent organization API and task-scope implementation | Required to let GTC-operated and external agents submit creation requests for client objects while granting management access only after verified authority evidence and active object assignment. Runtime migration 020, `/agents/` page shell and CPG-BIZ-115 API plan are implemented; API skeleton/task computation remain open. |
...
1. Platform Administration / Control API and UI for agent organizations, authority documents and object creation requests;
