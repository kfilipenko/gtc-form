...
- Version: 3.19
- Date: 2026-07-08
- Status: Synchronized after CPG-BIZ-138 agent contract workflow menu implementation
...
| CPG-BIZ-135 | Unified contract workspace for agent and direct party agreements | Fixed that direct seafarer-shipowner, shipowner-agent and future seafarer-agent agreements must use one contract workspace/template/generation mechanism with explicit `contract_kind`, source adapters, appendix registry, party approvals and generated snapshots |
| CPG-BIZ-136 | Contract workspace multi-template registry / contract-kind prerequisite | Reserved next technical layer from CPG-BIZ-135: template lookup by `contract_kind` / `template_code`, generalized source/party adapters and appendix checklist support. |
| CPG-BIZ-137 | Agent-assisted direct contract drafting | Implemented first runtime slice: authorized agent tasks open exact eligible direct SEA workspaces; agent scope is checked against assignment, authority and workspace source objects; permitted embedded fields can be saved with preview hash/audit evidence; draft can be sent to real parties through approval-request rows and participant notification ledger records. |
| CPG-BIZ-138 | Agent contract workflow menu and workspace | Implemented shared `Agents` menu contract accordion and `/agents/contracts/` workflow page for prepare contract, agree terms, edit draft and send to parties for approval, all nested under `Contracts` and using computed task targets with the guarded Contract Workspace. |
...
| Framework agreement and commercial terms setup | CC-08 / CF-05 | Framework agreement acceptance, authority package, required appendices/evidence, Service Order / commercial addendum / request, service entitlement | Group 3 billing/commercial, Group 5 control, Group 1 client intake | BP-015, BP-012, CPG-BIZ-123, CPG-BIZ-132, CPG-BIZ-134, CPG-BIZ-135 | Partial | Runtime API/UI must separate `framework_terms_accepted` and `commercial_terms_pending/service_order_accepted`: legal representative management may activate after authority and authoritative English package acceptance, but paid service, billing basis, success fee and SLA require separate commercial agreement. The next slice must represent framework agreement, authority/POA and Service Order/commercial addendum as contract workspace template/appendix/evidence records rather than standalone text snapshots. |
...
| Contract Agreement Workspace | CC-14 / CF-14 | Populated agreement with embedded condition fields, contract kind, source adapters, agent-assisted draft versions, required appendices and party approvals | Group 1, Group 4, Group 5, represented agent where scoped | CPG-BIZ-091/092/093/094/095/096/097/098A/098B/098C/098D/099/100/102, CPG-BIZ-125, CPG-BIZ-132, CPG-BIZ-134, CPG-BIZ-135, CPG-BIZ-136, CPG-BIZ-137, CPG-BIZ-138, BP-014, ICS-005 | Implemented for direct SEA foundation: workspace model, clause library, catalogs, source-first prefill rule, exact shortlist candidate link, runtime schema migration 018, employer `propose_contract` API/UI creation/reuse, detail view and verified source-prefill page `/contracts/workspace/`. First CPG-BIZ-137 runtime slice is implemented: `/agents/tasks` and `/agents/contract-drafting/tasks` compute agent-visible contract drafting tasks, `/contracts/workspace/` opens with `actor=agent&assignment_id=...`, permitted embedded fields can be saved, `preview_hash` and audit evidence are updated, and the populated draft can be sent to real parties for review through approval-request rows and notification ledger records. CPG-BIZ-138 adds the shared agent menu and `/agents/contracts/` workflow page for prepare contract, agree terms, edit draft and send to parties, preserving computed task targets. CPG-BIZ-135 identifies that the current code is still SEA-specialized and must be generalized by `contract_kind` before agent agreements are generated through the same script. | Remaining work: party approval/correction endpoints and UI, simplified party review route, approval invalidation after draft changes, multi-template registry/API/UI, appendix/evidence checklist, dedicated draft-version table if required and final generated instance script. |
...
| P1 | Contract workspace multi-template registry and generation adapter | Required before adding agent agreements to automation; contract templates must be selected by `contract_kind` and generated through one workspace/script mechanism. |
| P1 | Agent-assisted direct contract drafting follow-up | First runtime slice is implemented under CPG-BIZ-137 / ICS-005. Remaining work must add party approval/correction endpoints and UI, simplified party review route, approval invalidation after draft change and final signature/generation gate integration. |
...
14. CPG-BIZ-132 - authoritative English shipowner-agent agreement package for runtime offer/acceptance and public legal publication.
...
|---|---|---|---|
| 3.19 | 2026-07-08 | GTC IT / AI Assistant | Added CPG-BIZ-138 implementation mapping: shared agent contract menu, `/agents/contracts/` workflow page and stage links nested under `Contracts` while preserving computed Contract Workspace task targets |
| 3.18 | 2026-07-08 | GTC IT / AI Assistant | Marked the first CPG-BIZ-137 runtime slice as implemented: agent contract drafting tasks, exact workspace link, assignment/authority/source guard, permitted embedded-field save, preview hash/audit update and party-review notification request |
| 3.17 | 2026-06-19 | GTC IT / AI Assistant | Added CPG-BIZ-137 agent-assisted direct contract drafting task and mapped CF-14 to agent-scoped direct SEA draft versions, party review and approval guards; kept CPG-BIZ-136 reserved for the contract-kind registry prerequisite |
| 3.16 | 2026-06-19 | GTC IT / AI Assistant | Added CPG-BIZ-135 unified contract workspace direction for direct SEA, shipowner-agent and future seafarer-agent agreements, including multi-template registry and appendix/evidence checklist gap |
