...
| CF-08A Incoming seafarer request review | `review_candidate_presentation` with `request_source=seafarer_initiated_request` | `review_team` | `approve_candidate_presentation` | Review-team user opens the concrete vacancy application, sees it as `Review incoming seafarer request`, then releases it to presented-candidate workflow, requests correction or rejects it; shipowner incoming-request visibility is recomputed after completion. |
