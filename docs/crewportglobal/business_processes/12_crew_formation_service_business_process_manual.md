...
| CF-08A Seafarer job-search counter-flow | `find_matching_vacancies` / request contract consideration | Seafarer owner, then existing review/employer flow | owner cabinet/profile access; later `view_review_queue` or employer candidate access | Seafarer sees matching published verified vacancies from the saved profile, may request contract consideration for an eligible vacancy, and the request is recorded as a controlled `vacancy_applications` row without bypassing review, shortlist, presentation or contract guards. |
