...
| CF-08A Incoming seafarer request review | `review_candidate_presentation` with `request_source=seafarer_initiated_request` | `review_team` | `approve_candidate_presentation` / structured correction-rejection reason | Review-team user opens the concrete vacancy application, sees it as `Review incoming seafarer request`, then releases it to presented-candidate workflow, requests correction with a structured reason or rejects it with a structured reason; shipowner incoming-request visibility is recomputed after completion without exposing internal rationale. |
