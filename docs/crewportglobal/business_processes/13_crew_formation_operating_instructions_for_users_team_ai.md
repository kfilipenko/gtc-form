...
| Seafarer job-search request | seafarer owner, shipowner owner, then existing review/employer groups | owner profile/cabinet access; shipowner incoming-request task; later existing review/employer permissions | The seafarer may request contract consideration only from a concrete matching vacancy. The request must create or reuse a controlled application/context record, compute a safe shipowner incoming-request task, and must not directly create employment status, a contract, invoice or employer-facing unrestricted data. |
