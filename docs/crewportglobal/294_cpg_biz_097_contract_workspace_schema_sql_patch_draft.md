...
| `candidate_presentation_id` | `vacancy_application_id` referencing `crewportglobal.vacancy_applications(vacancy_application_id)` |
...
1. whether `contract_workspace_instances.vacancy_application_id` is the right link for candidate-presentation state;
2. whether `document_id` in `generated_contract_instances` should reference `uploaded_documents` or a future generated-documents table;
3. whether `contract_field_catalogs` should reuse generic `reference_catalogs` later or remain contract-specific;
4. whether one generated active contract per workspace is sufficient;
5. whether `workspace_number` should be generated by backend code or database sequence;
6. whether party approvals require additional legal/e-signature provider metadata.
