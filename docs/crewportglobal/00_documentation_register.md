...
docs/crewportglobal/
  ...
  273_cpg_biz_078_translation_publication_workflow_command_report.md
  ...
  320_cpg_biz_119_agent_managed_object_workspace_report.md
  321_cpg_biz_120_structured_shipowner_agent_agreement_ru_template.md
  322_cpg_biz_121_shipowner_agent_agreement_ru_v1_1_commercial_model.md
  ...
  implemented_code_standards/
    ...
    03_standard_submit_review_gate.md
```
...
Document 182 records the CPG-DEMAND-019 team workbench computed tasks implementation, including named-session-only `My tasks` on `/team/`, read-only task derivation from current queue and shortlist data, `review_team` entry alignment, permission-filtered task visibility and preserved no-persisted-task / no-employer-facing boundary.

Document 183 records the CPG-DEMAND-020 team task deep-link implementation, including computed task `target_url` generation, `/team/` task links to specific `/verify/` records, query-parameter target parsing, automatic operator lane/filter/detail opening and preserved backend access/guard boundary.
...
Document 188 records the CPG-DEMAND-025 account-session team access recovery implementation, including `/verify/` and `/team/` loading through `cpg_user_session`, group-scoped queue/task visibility, PostgreSQL boolean normalization, account actor context, preserved operation permission checks and focused API/UI verification.
...
Document 193 records the CPG-DEMAND-030 homepage registry data access correction, including a first-screen Show registry data action, moving the safe live registry before explanatory service-model content, expanding preview rows per registry object and preserving the contact-free investor demonstration boundary.
...
Document 196 records the CPG-DEMAND-033 controlled comparison-to-shortlist handoff, including guarded internal shortlist draft creation from `/team/matching/`, default include/hold decisions based on candidate-search blockers, backend guard authority, draft task link and focused verification that no employer visibility or contact-data exposure is introduced.
...
Document 262 records the CPG-BIZ-067 Google translation provider boundary implementation, including backend/build-only adapter placeholder, public-tree credential scanning and confirmation that no Google credentials or browser-side provider calls are exposed.
...
Document 266 records the CPG-BIZ-071 Google translation dependency and protected environment readiness gate, including optional Google dependency isolation, local non-blocking readiness checks and strict protected-environment validation.

Document 267 records the CPG-BIZ-072 protected Google provider smoke-test plan and script, including one-key in-memory translation, readiness pre-blocking and no repository cache mutation.
...
| 3.30 | 2026-07-08 | GTC IT / AI Assistant | Added document 340 as CPG-BIZ-138 for the shared agent contract workflow menu and `/agents/contracts/` workspace: prepare contract, agree terms, edit draft and send to parties for approval |
...
| 3.11 | 2026-06-08 | GTC IT / AI Assistant | Added document 320 as CPG-BIZ-119 implementation report for agent-managed object participant cards and scoped object workspace |
...
| 2.91 | 2026-06-05 | GTC IT / AI Assistant | Added document 302 as CPG-BIZ-101 implementation report for the Shipowners candidate-selection workspace, safe presented-candidate visibility and guarded contract handoff |
...
| 2.60 | 2026-06-01 | GTC IT / AI Assistant | Added document 273 for the one-command translation runtime bundle publication workflow |
