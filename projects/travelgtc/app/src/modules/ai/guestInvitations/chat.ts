...
export class BoundInvitationChat implements ChatInvitationService{
 ...
 async handle(question:string,serverContactId:string):Promise<ChatInvitationResult|null>{
  ...
  if(r.status==='disabled')return result('Выдача персональных гостевых приглашений сейчас недоступна. Можно продолжить знакомство с условиями на TravelGTC и вернуться за приглашением позже.');
  if(r.status!=='assigned')return result(r.status==='already_redeemed'?'Ранее закреплённое за вами приглашение отмечено как использованное. Новое автоматически не выдаю.':r.status==='empty'?'Сейчас в пуле нет подтверждённого свободного приглашения. Нужна проверка и пополнение пула. Пока можно ознакомиться с условиями на TravelGTC; повторный подбор отдыха для этого не нужен.':r.status==='expired'?'Приглашение отмечено как истёкшее. Попросите заменить гостевой код — я проверю доступность замены.':'Сейчас сервис не смог проверить доступность гостевого приглашения. Непроверенный код выдавать не буду. Попробуйте запросить приглашение позже; ваши пожелания по поездке повторять не нужно.');
