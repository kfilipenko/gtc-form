...
const travel = /путешеств|поездк|отпуск|отел|гостиниц|маршрут|курорт|пляж|море|погода|виз[аыуе]|перел[её]т|авиабилет|дайв|трансфер|экскурс|достопримеч|ресторан|брониров|жиль[еёя]|жилья|страховк|круиз|вилл|чемодан|багаж|дахаб|египет|шарм|мальдив|бунгало|турци/iu;
export function travelHandoff(question: string, history: Turn[]): string | null {
 if(project.test(question)||/(?:^|\s)(?:не хочу|не надо|стоп|спасибо|пока|другая тема)(?:\s|[.!?,]|$)/iu.test(question))return null;
 let completed=0,pending=false,context=false;
 for(const turn of history){
  if(turn.role==='user'){
   const related: boolean=!project.test(turn.content)&&(travel.test(turn.content)||context);
   pending=related;context=related;
  }else if(turn.role==='assistant'&&pending){completed++;pending=false;}
 }
 if(completed<4||!(travel.test(question)||context))return null;
