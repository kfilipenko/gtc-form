...
export function discussionSubject(history,question){
 ...
 for(const q of [...history.filter(t=>t.role==='user').map(t=>t.content),question]){
  if(/(?:бизнес|заработок).{0,20}(?:не\s+(?:интерес|нуж|хочу))|не\s+(?:надо|хочу|предлагай|предлагайте).{0,20}бизнес|никаких продаж|тему бизнеса закроем/iu.test(q))partner=false;
  else if(/(?:хочу|интересно|интересует).{0,70}(?:Ambassador|зарабатывать|бизнес|рекомендаци.{0,20}работ)|Ambassador.{0,35}(?:как\s+бизнес|как\s+работ)/iu.test(q))partner=true;
...
export function validateGrounding(d){
 ...
 if(/(?:покупка|приобретение).{0,30}(?:дорогого|дорогостоящего)\s+членства\s+не\s+является\s+обязательн/iu.test(t))errors.push('CURRENT_AMBASSADOR_ENTRY_REQUIREMENTS_UNKNOWN');
 if(/(?:ежегодн|ежемесячн)[а-яё]*.{0,25}(?:взнос|плат[её]ж)|(?:взнос|плат[её]ж).{0,25}(?:ежегодн|ежемесячн)/iu.test(t))errors.push('BILLING_PERIOD_NOT_VERIFIED_USE_CURRENT_PARTICIPATION_COSTS_ONLY');
 return errors;
}
